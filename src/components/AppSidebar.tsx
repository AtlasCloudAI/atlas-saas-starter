            ...
            return (
              <Link key={app.href} href={app.href} className={`flex gap-3 rounded-xl border p-3 transition ${activeClass(active)}`}>
                ...
                <span className="min-w-0">
                  <span className="block truncate text-sm font-semibold">{text.title}</span>
                  <span className="mt-0.5 block line-clamp-2 text-xs leading-4 text-neutral-400">{text.description}</span>
                </span>
              </Link>
            );
          })}
